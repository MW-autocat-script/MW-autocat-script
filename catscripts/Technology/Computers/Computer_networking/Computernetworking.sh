...
egrep -i 'Computer network|Ethernet|Wi(| |-)fi|wire(d|less) network|wireless card|(wire(d|less)|Ethernet) router|\bLAN(|s)\b|\bWAN(|s)\b|\bWLAN(|s)\b|\bNIC(|s)\b|VLAN|DHCP|network administrator|network administration|\bBOOTP|PXE|\bDNS\b|\bDDoS\b|\bTCP\b|\bIPX\b|IP address|OSPF|EIGRP' newpages.txt | egrep -iv 'SSH|Telnet|Lan Hikari|Obi( |-)wan|FTP|VoIP|Skype' >> Computernetworking.txt
