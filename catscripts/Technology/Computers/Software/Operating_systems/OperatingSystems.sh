...
cat newpages.txt | egrep -i 'operating system| OS\b|\[\[OS ' | egrep -iv 'Windows|Unix|Linux|Mac OS|Open(| )BSD|Net(| )BSD|Free(| )BSD|\bDOS\b|Google Android' >> OperatingSystems.txt
