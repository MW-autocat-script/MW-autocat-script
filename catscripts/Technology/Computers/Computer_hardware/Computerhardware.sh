...
KEYWORDS_DATASTORAGE="Data(| )storage|(USB|Flash)(| )drive|memory(| )stick|SD(| )card|SDHC|\bmicro(| )SD|(CD|DVD)(| )drive|(CD|DVD|BD)(| |-|\+)(ROM|R(|W)|burner|writer)|Blu(| |-)ray(| )drive|SCSI|\b(|e)SATA\b|\bATA(|PI)\b|\bPATA\b|IDE(| )drive|Compact(| )Flash"
