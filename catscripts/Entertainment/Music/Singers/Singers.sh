...
egrep -i "$KEYWORDS_SINGERS_KESHA" newpages.txt >> Kesha.txt
...
egrep -i "$KEYWORDS_SINGERS_ASTONMERRYGOLD" newpages.txt >> AstonMerrygold.txt
egrep -i "$KEYWORDS_SINGERS_ELTONJOHN" newpages.txt >> EltonJohn.txt
egrep -i "$KEYWORDS_SINGERS_NICKIMINAJ" newpages.txt >> NickiMinaj.txt
...
MINAJ=`stat --print=%s NickiMinaj.txt`
MERRYGOLD=`stat --print=%s AstonMerrygold.txt`
